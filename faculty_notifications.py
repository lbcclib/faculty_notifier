...
spanish_language = Department('Spanish Language', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', ' ')

departments.append(Department('Agriculture/Horticulture department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(HD140[1-9]|HD14[1-9][0-9]|HD1[5-9][0-9]{2}|HD2[01][0-9]{2}|HD220[0-9]|HD2210|S[0-9]+|SB[0-9]+|TJ148[0-9]|TJ149[0-6]|QK[0-9]+).*'))
departments.append(Department('Biology department', 'alberto.martnz@gmail.com', r'^(GE[0-9]|QH[0-9]|QK[0-9]|QL[0-9]|QM[0-9]|QP[0-9]|QR[0-9]|TP248\.1[3-9]|TP248\.[2-5]|TP248\.6[0-5]).*'))
departments.append(Department('Business department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(HB[0-9]|HC[0-9]|HD[0-9]|HE[0-9]|HF[0-9]|HG[0-9]|HJ[0-9]).*'))
...
departments.append(Department('Chemistry department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^QD.*'))
...
departments.append(Department('Criminal justice department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(HV[678][0-9]{3}|HV9[0-8][0-9]{2}|HV99[01][0-9]|HV9920|KF9[67][0-9]{2}|KF98[01][0-9]|KF982[0-7]).*'))
departments.append(Department('Dental assistant department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^RK.*'))
...
departments.append(Department('English/Writing department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(LB2360|PE140[2-9]|PE14[1-8][0-9]|PE149[0-7]|PN335[5-9]|PN33[67][0-9]|PN338[0-5]|T11|T11\.[0-5]|ZA3075)(\.|\s).*'))
departments.append(Department('Health and Human Performance / PE Department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(GV20[1-9]|GV2[1-9][0-9]|GV[3-9][0-9]{2}|GV10[0-9]{2}|GV11[0-8][0-9]|GV119[0-8]|RC12[0-3][0-9]|RC124[0-5])(\.|\s).*'))
departments.append(Department('History department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^([CDEF][0-9]+)(\.|\s).*'))
departments.append(Department('Journalism department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^PN(4699|4[7-9][0-9]{2}|5[0-5][0-9]{2}|56[0-5][0-9]).*'))
departments.append(Department('Machine tool', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(TJ11[89][0-9]|TJ1[012][0-9]{2}|TJ130[0-9]|TJ131[0-3]).*'))
departments.append(Department('Math department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(HA|QA[1-6]|QA7[0-5]|QA7[89]|QA[89]).*'))
...
departments.append(Department('Physics department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^Q[BC][0-9]+.*')) 
departments.append(Department('Psychology department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(BF[0-9]+|R726\.[5-8]|RC32[1-9]|RC3[3-9][0-9]|RC4[0-9]{2}|RC5[0-6][0-9]|RC57[01])(\.|\s).*'))
departments.append(Department('Religion department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(B[LMPQRSTVX][0-9]+|DS14[0-9]|KB[0-9]+)(\.|\s).*'))
departments.append(Department('Small Business Development Center', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(HD67\.2|HD2340\.[89]|HD234[0-5]|HD2346\.[0-5]|HF5679).*'))
departments.append(Department('Sociology department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(E184|H[MNQSTV][0-9]+).*'))
departments.append(Department('Spanish department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(PC407[3-9]|PC4[1-8][0-9]{2}|PC49[0-6][0-9]|PC497[01]|PQ603[7-9]|PQ60[4-9][0-9]|PQ6[1-7][0-9]{2}|PQ8[0-4][0-9]{2}|PQ85[01][0-9]).*'))
departments.append(Department('Theater department', 'alberto.martinezsanchez.6407@mail.linnbenton.edu', r'^(PN15[3-9][0-9]|PN1[6-9][0-9]{2}|PN[23][0-9]{3}|PN4[0-2][0-9]{2}|PN43[0-4][0-9]|PN435[0-5]).*'))
...
feed_url = 'http://' + config['opac_host'] + '/opac/extras/browse/atom-full/item-age/' + config['org_unit'] + '/1/' + str(config['num_items_to_fetch']) + '?status=5' # the feed element 
...
for book in books: # now that we have data from the book that are being saved in 'books'
	possible_book = Book(book)
        if (hasattr(possible_book, 'spanish') and (possible_book.has_image())):
                spanish_language.mark_book_for_email (possible_book)
                print(possible_book.spanish)
                print(possible_book.title)
...
if spanish_language.has_enough_data_for_email():
          spanish_language.send_email()
