...
departments.append(Department('Biology department', 'sandbej@linnbenton.edu', r'^(GE[0-9]|Q[1-9]|Q[1-9][0-9]|Q[12][0-9][0-9]|QH[0-9]|QK[0-9]|QL[0-9]|QM[0-9]|QP[0-9]|QR[0-9]|TP248\.1[3-9]|TP248\.[2-5]|TP248\.6[0-5])([0-9]|\.|\s).*'))
...
departments.append(Department('Criminal justice department', 'sandbej@linnbenton.edu', r'^(HV[678][0-9]{3}|HV9[0-8][0-9]{2}|HV99[01][0-9]|HV99[25]0|KF9[67][0-9]{2}|KF98[01][0-9]|KF982[0-7]).*'))
...
departments.append(Department('Education and HDFS department', 'sandbej@linnbenton.edu', r'^(GN480|GT24[6-8][0-9]|GT2490|HV699|HV[7-9][0-9]{2}|HV1[0-3][0-9]{2}|14[0-4][0-9]|HQ75\.27|HQ75\.53|HQ77[789]|HQ78[01]|HQ79[3-9]|HQ5[0-9]{2}|L[ABC]?[0-9]+)(\.|\s).*'))
departments.append(Department('ELA department', 'sandbej@linnbenton.edu', r'^(LB1572|LB1576|P11[89]|P12[01]|PE110[89]|PE111[0-4]|PE1128\.A2|PE113[1-9]|PE1404|PE1449)(\.|\s).*'))
departments.append(Department('Engineering department', 'sandbej@linnbenton.edu', r'^(T[AJK][0-9]+|TP15[56])(\.|\s).*'))
departments.append(Department('English/Writing department', 'sandbej@linnbenton.edu', r'^(LB2360|PE140[2-9]|PE14[1-8][0-9]|PE149[0-7]|PN335[5-9]|PN33[67][0-9]|PN338[0-5]|T11|T11\.[0-5]|ZA3075)(\.|\s).*'))
...
departments.append(Department('Nursing and CNA department', 'sandbej@linnbenton.edu', r'^(Q[MP]|R[GST])[0-9]+.*'))
...
departments.append(Department('Physics department', 'sandbej@linnbenton.edu', r'^(Q[1-9]|Q[1-9][0-9]|Q[12][0-9][0-9]|Q[BC])([0-9]|\.|\s).*')) 
...
departments.append(Department('Religion department', 'sandbej@linnbenton.edu', r'^(B[LMPQRSTVX][0-9]+|DS14[0-9]|KB[0-9]+)(\.|\s).*'))
...
departments.append(Department('Sociology department', 'sandbej@linnbenton.edu', r'^(E184|H[NQSTV][0-9]+|HM[1-9]|HM[1-9][0-9]|HM[1-9][0-9]{2})(\.|\s).*'))
